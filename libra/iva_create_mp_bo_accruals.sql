create
    definer = iva@`%` procedure iva_create_mp_bo_accruals(in current_order_id  int,
      ...
      join (
        with parent_go (user_id, value) as (
          ...
          box_go (user_id, value) as (
            ...
            where
              bonus.source_user_id = bonus.target_user_id
              and bonus.type = 'sale'
          ...
          base_go (parent_id, parent_go, child_id, type) as (
            select
              ...
              ifnull(child_next.parent_id, child_current.child_id) current_child_id,
              'user' type
            from
              iva_mp_transactions bonus
