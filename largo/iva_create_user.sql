...
begin
  ...
  on duplicate key update
      ...
      largo.iva_users.activation = user_props.uf_has_activation = true
        and ( user_props.uf_activation_till is null
        or user_props.uf_activation_till >= iva_get_period_finish(current_period_id) ),
      largo.iva_users.email_confirmed = users.email_confirmed,
      largo.iva_users.available_amount = iva_get_available_balance(users.id);

  call largo.iva_create_user_requisites(current_user_id);
end;
